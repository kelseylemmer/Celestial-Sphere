...
export const UserList = () => {
  ...
  const addToMySphere = (profileId) => {
    ...
    const alreadyAdded = addedProfiles.some(
      (addedProfile) => addedProfile.addedProfileId === profileId
    );

    if (alreadyAdded) {
      // Profile already added, perform delete operation
      const addedProfileId = addedProfiles.find(
        (addedProfile) => addedProfile.addedProfileId === profileId
      ).id;

      fetch(`http://localhost:8088/userSpheres/${addedProfileId}`, {
        method: "DELETE"
      })
        .then(response => response.json())
        .then(() => {
          ...
          const updatedAddedProfiles = addedProfiles.filter(
            (addedProfile) => addedProfile.id !== addedProfileId
          );
          ...
          filteredProfiles.map((profile) => (
            <section className="user" key={`${profile.id}`}>
              ...
              <section className="bottom">
                ...
                >
                  {addedProfiles.some(
                    (addedProfile) => addedProfile.addedProfileId === profile.id
                  )
                    ? "☹ Delete"
                    : "✢ Add"}
                </button>
